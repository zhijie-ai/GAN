...
#               不胜人生一场醉。                 #
#----------------------------------------------
from GAN.SSGAN.impl3_tf.vlib.layers import *
from GAN.SSGAN.impl3_tf.vlib.load_data import *
import os
import GAN.SSGAN.impl3_tf.vlib.plot as plot
import GAN.SSGAN.impl3_tf.vlib.save_images as save_img
import time
...
class Train(object):
    ...
    def build_model(self):
        ...
        # caculate the unsupervised loss(无监督学习也是要区分真假样本的)
        ...
        # 因为此时是真样本，它不知道具体是哪一个类别。最后一个神经元应该输出较小的值，假样本时，输出较大的值
        ...
        d = tf.nn.sigmoid_cross_entropy_with_logits(labels=un_label_f * 0.9, logits=d_logits_f)
        print('TTTTTTTTTTTT',d.shape)
        # feature match
        ...
        self.d_loss = d_loss_r + d_loss_f + s_l_r*self.flag*10 + d_regular #(d_loss_r是没有负号的)
        self.g_loss = d_loss_f + 0.01*f_match

        # 下面参照的是另一种实现方法，相当于把监督学习和无监督学习分开来看了。注意和keras的区别。keras也分开了，但训练的
        # 时候要一起训练。tf可以分开训练
        # self.label = tf.placeholder(tf.float32, shape=[self.batch_size, self.num_class - 1], name='label')
...
# output = conv2d('Z_cona{}'.format(i), output, 3, 64, stride=1, padding='SAME')
    ...
    def discriminator(self, name, inputs, reuse):
        ...
        with tf.variable_scope(name,reuse=reuse):
            ...
            output = fc('d_fc', output, self.num_class) # num_class=11
