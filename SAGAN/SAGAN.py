...
class SAGAN():
    ...
    def generator(self,z,is_training=True,reuse=False):
        with tf.variable_scope('generator',reuse=reuse):
            ch = 1024
            x=deconv(z,channels=ch,kernel=4,stride=1,padding='VALID',
                     use_bias=False,sn=self.sn,scope='deconv')
            ...
            for i in range(self.layer_num//2):
                if self.up_sample:
                    ...
                    x=conv(x,channels=ch//2,kernel=3,stride=1,pad=1,
                           sn=self.sn,scope='up_conv'+str(i))
    ...
    def discriminator(self,x,is_training=True,reuse=False):
        with tf.variable_scope('discriminator',reuse=reuse):
            ch=64
            print('before',x.shape.as_list()) #  [32, 128, 128, 1]
            x=conv(x,channels=ch,kernel=4,stride=2,pad=1,sn=self.sn,use_bias=False,scope='conv')
            print('after',x.shape.as_list()) # [32, 64, 64, 64]
            x=lrelu(x,0.2)

            for i in range(self.layer_num//2):
                x=conv(x,channels=ch*2,kernel=4,stride=2,pad=1,
                       sn=self.sn,use_bias=False,scope='conv_'+str(i))
                x = batch_norm(x,is_training,scope='batch_norm_'+str(i))
                x=lrelu(x,0.2)
            ...
            print('BBBBBBBBBBBBB',x.shape.as_list())
            return x

    def attention(self,x,ch,sn=False,scope='attention',reuse=False):
        print('CCCCCCCCCCC', ch) # x.shape [32, 16, 16, 256]
        with tf.variable_scope(scope,reuse=reuse):
            f = conv(x,ch//8,kernel=1,stride=1,sn=sn,scope='f_conv') # [bs,h,w,c][32, 16, 16, 32]
            g = conv(x,ch//8,kernel=1,stride=1,sn=sn,scope='g_conv') # [bs,h,w,c]
            h = conv(x,ch,kernel=1,stride=1,sn=sn,scope='h_conv') # [bs,h,w,c][32, 16, 16, 256]
            print('f.shape',f.shape.as_list(),h.shape.as_list())

            # N=h*w
            print('DDDDDDDDDDDDDD',hw_flatten(g).shape.as_list())
            print('DDDDDDDDDDDDDDE', hw_flatten(f).shape.as_list())
            ...
            o = tf.reshape(o,shape=x.shape) # [bs,h,w,c] [32, 16, 16, 256]
            print('RRRRRRRRRR',o.shape.as_list(),x.shape.as_list())
            x= gamma*o +x
    ...
    def visualize_results(self, epoch):
        ...
        save_images(samples[:image_frame_dim * image_frame_dim, :, :, :],
                    ...
                    self.sample_dir + '/' + self.model_name + '_epoch%02d' \
                        % epoch + '_visualize.png')
